...
<template>
  ...
  >
    <template #actions>
      <AppButton v-if="data" type="button" class="btn-ghost" @click="expandAll">
        <AppIcon name="chevronDown" :size="13" />Mở tất cả
      </AppButton>
      <AppButton v-if="data" type="button" class="btn-ghost" @click="collapseAll">
        <AppIcon name="chevronRight" :size="13" />Thu tất cả
      </AppButton>
      <AppButton type="button" class="btn-export" @click="ui.showToast({ icon: 'download', title: 'Xuất báo cáo kho' })">
        <AppIcon name="download" :size="15" />Báo cáo
      </AppButton>
      <AppButton type="button" class="btn-add" @click="ui.openModal('stockIn')">
  ...
  <div v-if="data" class="inventory-groups">
    <div v-for="group in groupedProducts" :key="group.productId" class="inv-group">
      ...
      <AppButton type="button" class="inv-group-header" @click="toggleProduct(group.productId)">
      ...
      <div v-if="isExpanded(group.productId)" class="inv-variant-list">
        <div v-for="row in group.variants" :key="row.variantId" class="inv-variant-row">
          ...
          <span class="ivc ivc-actions">
            <AppButton type="button" class="ra-btn ra-edit" @click="ui.openModal('stockIn', row)">
