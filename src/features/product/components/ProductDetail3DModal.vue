...
async function loadModel() {
  ...
  try {
    ...
    loadError.value = 'Không tải được mô hình 3D cho sản phẩm này.'
  } finally {
    ...
    <div class="box">
      ...
      <div class="scene">
        <div class="scene-controls">
          <button type="button" class="scene-ctrl-btn" :class="{ active: viewMode === 'product' }" @click="viewMode = 'product'">
            Sản phẩm
          </button>
          <button type="button" class="scene-ctrl-btn" :class="{ active: viewMode === 'room' }" @click="viewMode = 'room'">
            Phòng đầy đủ
          </button>
        </div>

        <div v-if="viewMode === 'product' && supports3d" ref="viewportRef" class="scene-viewport"></div>
        <div v-else-if="viewMode === 'product'" class="scene-empty">
          Mẫu này chưa có mô hình 3D thật. Bạn vẫn có thể xem thông tin và đặt trong Room3D.
        </div>
        <div v-else class="scene-room-mode">
          <p>Để xem sản phẩm trong không gian phòng, chuyển qua Trực quan 3D và đặt vào phòng mẫu.</p>
          <button type="button" class="room-cta-btn" @click="emit('go-room3d', { roomType: roomTypeHint })">
            Mở Trực quan 3D
          </button>
        </div>
